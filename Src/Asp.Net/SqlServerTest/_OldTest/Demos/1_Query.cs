...
namespace OrmTest.Demo
{
    public class Query : DemoBase
    {
        ...
        public static void Easy()
        {
            ...
            var test46 = db.Queryable<Student>().Where(it => it.CreateTime > SqlFunc.ToDateTime(DateTime.Now.Date)).ToList();
            var test47 = db.Queryable<Student>().Where(it =>string.IsNullOrEmpty(it.Name)==true).ToList();
            var test48 = db.Queryable<Student>().Where(it=>it.CreateTime!=null).Where(it => SqlFunc.ToDateTime(it.CreateTime).Date==DateTime.Now.Date).ToList();
            var test49 = db.Queryable<Student>().Where(it => it.CreateTime != null).Where(it => SqlFunc.ToDateTime(it.CreateTime).Year == DateTime.Now.Year).ToList();
            var test50 = db.Queryable<Student>().Where(it => it.CreateTime != null).Where(it => SqlFunc.ToDateTime(it.CreateTime).Year == SqlFunc.GetDate().Year).ToList();
            var test51 = db.Queryable<Student>().Select(it=>new { x= SqlFunc.ToDateTime(it.CreateTime).Year+"-" }).ToList();
